...
# どうやって外部キャッシュフローの価値を確定するの？

キャッシュフローの価値が[標準資産]({{ site.baseurl }}/tables_and_views_jp.html#standard_asset)で計算されます。ポートフォリオ中の資産が標準資産(自国通貨)の形式で流入・流出している場合、価値はすぐに確定します。しかし、標準資産の形式でない流入や流出が行われる場合があります。

ある取引を例として挙げます。

| 流出のアカウント | 流出の資産 | 流入のアカウント | 流入の資産 |
|:-:|:-:|:-:|:-:|
| A | -500 | B | 10 |

ポートフォリオにアカウント`A`のみが含まれている場合、この取引は資金の流出となり、ポートフォリオにアカウント`B`のみが含まれている場合、この取引は資金の流入となります。 アカウント`A`とアカウント`B`は両方とも非標準資産が含まれていると仮定すると、この取引では標準資産建ての流入·流出の価値を決定する必要があります。

現実にもこのような取引に相当するシーンは、自国通貨以外への投資であります。例えば、自国通貨は日本円ですが、米ドルでアメリカの株を購入する場合です。 このような取引では、日本円での流入·流出の価値を確定するときに、正確な方法は、取引の瞬間の米ドルと日本円の為替レートを記録し、それを換算するのです。

しかし、TataruBookの[prices]({{ site.baseurl }}/tables_and_views_jp.html#prices)テーブルには、各日終了時間の資産価格の情報しか含まれておらず、取引時の即時価格は含まれていません。そのため、この取引の流入·流出の価値について、TataruBookの計算の正確さは完全ではありません。しかし、ほとんどの場合、資産価格は一日中あまり変動しないことを考えると、この近似値一般的には十分です。

一般的に、TataruBookのアルゴリズムによって、アカウント`A`の流出の価値を計算する際に、アカウント`B`の流入の金額を当日アカウント`B`に含まれる資産の単位価格に変換します。それに対して、アカウント`B`の流入の価値を計算する際に、アカウント`A`の流出の金額を当日アカウント`A`に含まれる資産の単位価格に変換します。

ただし、このアルゴリズムが適用しない特殊な取引があります。それは、取引の片方のアカウントの変動額が$$ 0 $$である場合です。

| 流出のアカウント | 流出の資産 | 流入のアカウント | 流入の資産 |
|:-:|:-:|:-:|:-:|
| A | 0 | B | 200 |

このような取引は通常、株式やファンドの配当金、あるいは債券のクーポンが支払われる場合にあります。 例えば、アカウント`A`がアメリカの株、アカウント`B`が配当金を受け取る米ドルのアカウントの場合、上記のアルゴリズムをそのまま使用すると、この取引において、アカウント`B`の流入額は$$ 0 $$となり、合理的ではありません。

この問題を解決するために、TataruBookでは特殊な処理をしています。取引の片方の変動額が$$ 0 $$である場合、変化額が$$ 0 $$でない側の資産のタイプや変動額に応じて、両方のアカウントの流入·流出の価値を確定します。 先ほどの例では、アカウント`A`の流出の価値とアカウント`B`の流入の価値は、全部$$ 200 $$×アカウント`B`に含まれる資産の当日の単位価格になります。 このプロセスは、[share_trade_flows]({{ site.baseurl }}/tables_and_views_jp.html#share_trade_flows)ビューで確認できます。

瞬時の単位価格を使用して流入·流出の価値を精確に計算する必要がある場合は、中間アカウント`X`を使用 して、取引記録を二件に分割します。
...
この例では、アカウント`X`は標準資産を含み、その変動額は取引の時点での流入·流出の価値を表します。取引記録をこのように入力することで、TataruBookは流入·流出の価値正に計算でき、[prices]({{ site.baseurl }}/tables_and_views_jp.html#prices)テーブルの価格情報を使う必要がなくなります。
