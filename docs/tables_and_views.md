...
- `end_amount`: the ending balance calculated by $$ \text{start\_amount} + \text{diff} $$.
...
- `profit`: the profit (or loss) on the investment in this account, calculated as $$ \text{cash\_gained} + \text{end\_value} - \text{start\_value} $$.
